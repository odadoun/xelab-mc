...
void XelabDetectorConstruction::DefineMaterials()
{
  ...
  LXe = new G4Material(
      "LXe", 2.862 * g / cm3, 1, kStateLiquid,
      177.05 * kelvin, 1.5 * atmosphere);
  LXe->AddElement(Xe, 1);

  // Optical properties of LXe
  const G4int iNbEntries = 3;
  G4double LXePhotonMomentum[iNbEntries] = {6.91 * eV, 6.98 * eV, 7.05 * eV};
  G4double LXeScintillation[iNbEntries] = {0.1, 1.0, 0.1};
  G4double LXeRefractiveIndex[iNbEntries] = {1.63, 1.61, 1.58};
  G4double LXeAbsorbtionLength[iNbEntries] = {100. * cm, 100. * cm, 100. * cm};
  G4double LXeScatteringLength[iNbEntries] = {30. * cm, 30. * cm, 30. * cm};
  G4MaterialPropertiesTable *LXeMPT = new G4MaterialPropertiesTable();
  LXeMPT->AddProperty(
      "FASTCOMPONENT", LXePhotonMomentum, LXeScintillation, iNbEntries);
  LXeMPT->AddProperty(
      "SLOWCOMPONENT", LXePhotonMomentum, LXeScintillation, iNbEntries);
  LXeMPT->AddProperty(
      "RINDEX", LXePhotonMomentum, LXeRefractiveIndex, iNbEntries);
  LXeMPT->AddProperty(
      "ABSLENGTH", LXePhotonMomentum, LXeAbsorbtionLength, iNbEntries);
  LXeMPT->AddProperty(
      "RAYLEIGH", LXePhotonMomentum, LXeScatteringLength, iNbEntries);
  LXeMPT->AddConstProperty("SCINTILLATIONYIELD", 0. / keV);
  LXeMPT->AddConstProperty("RESOLUTIONSCALE", 0);
  LXeMPT->AddConstProperty("FASTTIMECONSTANT", 3. * ns);
  LXeMPT->AddConstProperty("SLOWTIMECONSTANT", 27. * ns);
  LXeMPT->AddConstProperty("YIELDRATIO", 1.0);
  LXeMPT->AddConstProperty("TOTALNUM_INT_SITES", -1);
  ...
  GXe = new G4Material(
      "GXe", 0.005887 * g / cm3, 1, kStateGas,
      173.15 * kelvin, 1.5 * atmosphere);
  GXe->AddElement(Xe, 1);

  // Optical properties of GXe
  // const G4int iNbEntries = 3;
  G4double GXePhotonMomentum[iNbEntries] = {6.91 * eV, 6.98 * eV, 7.05 * eV};
  G4double GXeScintillation[iNbEntries] = {0.1, 1.0, 0.1};
  G4double GXeRefractiveIndex[iNbEntries] = {1.00, 1.00, 1.00};
  G4double GXeAbsorbtionLength[iNbEntries] = {100 * m, 100 * m, 100 * m};
  G4double GXeScatteringLength[iNbEntries] = {100 * m, 100 * m, 100 * m};
  G4MaterialPropertiesTable *GXeMPT = new G4MaterialPropertiesTable();
  GXeMPT->AddProperty(
      "FASTCOMPONENT", GXePhotonMomentum, GXeScintillation, iNbEntries);
  GXeMPT->AddProperty(
      "SLOWCOMPONENT", GXePhotonMomentum, GXeScintillation, iNbEntries);
  GXeMPT->AddProperty(
      "RINDEX", GXePhotonMomentum, GXeRefractiveIndex, iNbEntries);
  GXeMPT->AddProperty(
      "ABSLENGTH", GXePhotonMomentum, GXeAbsorbtionLength, iNbEntries);
  GXeMPT->AddProperty(
      "RAYLEIGH", GXePhotonMomentum, GXeScatteringLength, iNbEntries);
  GXeMPT->AddConstProperty("SCINTILLATIONYIELD", 0. / keV);
  GXeMPT->AddConstProperty("RESOLUTIONSCALE", 0);
  GXeMPT->AddConstProperty("FASTTIMECONSTANT", 3. * ns);
  GXeMPT->AddConstProperty("SLOWTIMECONSTANT", 27. * ns);
  GXeMPT->AddConstProperty("YIELDRATIO", 1.0);
...
void XelabDetectorConstruction::DefineGeometryParameters()
{
  ...
  geometryParameters["innCryoTopSealUpperLowerUnion_z"] = (geometryParameters["innCryoTopSealLowerPart_halfWidth"] +
  ...
  geometryParameters["innCryoTopSeal_z"] = (geometryParameters["innCryoEnvelope_halfLength"] -
  ...
  geometryParameters["innCryoTopEndCap_z"] = (geometryParameters["innCryoEnvelope_halfLength"] +
  ...
  geometryParameters["innCryoPipe_z"] = (geometryParameters["innCryoEnvelope_halfLength"] -
  ...
  geometryParameters["outCryoTopSealUpperSubPart_halfWidth"] = (2.75 * mm +
  ...
  geometryParameters["outCryoTopSealInnerLowerSubPart_halfWidth"] = (3.75 * mm +
                                                                     geometryParameters["booleanTolerance"]);
  ...
  geometryParameters["outCryoTopSealOuterLowerSubPart_halfWidth"] = (1.5 * mm +
                                                                     geometryParameters["booleanTolerance"]);

  geometryParameters["outCryoTopSealMainUpperSub_x"] = 0. * mm;
  geometryParameters["outCryoTopSealMainUpperSub_y"] = 0. * mm;
  geometryParameters["outCryoTopSealMainUpperSub_z"] = (geometryParameters["outCryoTopSealMainPart_halfWidth"] -
  ...
  geometryParameters["outCryoTopSeal_z"] = (geometryParameters["outCryoEnvelope_halfLength"] +
                                            geometryParameters["outCryoTopSealMainPart_halfWidth"] -
                                            7. * mm);
  ...
  geometryParameters["outCryoTopEndCapUpperSubPart_halfWidth"] = (1.5 * mm +
                                                                  geometryParameters["booleanTolerance"]);
  ...
  geometryParameters["outCryoTopEndCapInnerLowerSubPart_halfWidth"] = (1.5 * mm +
                                                                       geometryParameters["booleanTolerance"]);
  ...
  geometryParameters["outCryoTopEndCapOuterLowerSubPart_halfWidth"] = (3. * mm +
                                                                       geometryParameters["booleanTolerance"]);

  geometryParameters["outCryoTopEndCapMainUpperSub_x"] = 0. * mm;
  geometryParameters["outCryoTopEndCapMainUpperSub_y"] = 0. * mm;
  geometryParameters["outCryoTopEndCapMainUpperSub_z"] = (geometryParameters["outCryoTopEndCapMainPart_halfWidth"] -
  ...
  geometryParameters["outCryoTopEndCap_z"] = (geometryParameters["outCryoTopSeal_z"] +
                                              geometryParameters["outCryoTopSealMainPart_halfWidth"] +
                                              geometryParameters["outCryoTopEndCapMainPart_halfWidth"]);
  ...
  geometryParameters["outCryoTopJointOuterPart_innerRadius"] = (250.5 * mm -
  ...
  geometryParameters["outCryoTopJoint_z"] = (geometryParameters["outCryoTopSeal_z"] +
                                             geometryParameters["outCryoTopSealMainPart_halfWidth"] -
  ...
  geometryParameters["outCryoColumnSealUpperLowerUnion_z"] = (geometryParameters["outCryoColumnSealLowerPart_halfWidth"] +
  ...
  geometryParameters["outCryoColumnSeal_z"] = (geometryParameters["outCryoTopEndCap_z"] +
                                               geometryParameters["outCryoTopEndCapMainPart_halfWidth"] +
  ...
  geometryParameters["outCryoColumnEndCap_z"] = (geometryParameters["outCryoColumnSeal_z"] +
                                                 geometryParameters["outCryoColumnSealLowerPart_halfWidth"] +
  ...
  geometryParameters["outCryoColumn_z"] = (geometryParameters["outCryoColumnSeal_z"] +
                                           geometryParameters["outCryoColumnSealLowerPart_halfWidth"] -
  ...
  geometryParameters["copperEnvelopeJunction_x"] = (geometryParameters["copperEnvelopeJunction_rho"] *
                                                    std::cos(geometryParameters["copperEnvelopeJunction_phi"]));
  geometryParameters["copperEnvelopeJunction_y"] = (geometryParameters["copperEnvelopeJunction_rho"] *
  ...
  geometryParameters["copperEnvelopeLongFrame_halfX"] = (27.6120169755 * mm +
                                                         geometryParameters["booleanTolerance"]);
  geometryParameters["copperEnvelopeLongFrame_halfY"] = 0.5 * mm;
  geometryParameters["copperEnvelopeLongFrame_halfZ"] = 125. * mm;

  geometryParameters["copperEnvelopeLongFrame_x"] = (geometryParameters["copperEnvelopeJunction_x"] -
                                                     geometryParameters["copperEnvelopeLongFrame_halfX"] +
                                                     geometryParameters["booleanTolerance"]);
  geometryParameters["copperEnvelopeLongFrame_y"] = (geometryParameters["copperEnvelopeJunction_y"] -
  ...
  geometryParameters["copperEnvelopeShortFrame_halfX"] = (10.1120169755 * mm +
                                                          geometryParameters["booleanTolerance"]);
  ...
  geometryParameters["copperEnvelopeShortFrame_x"] = -(
      ...
      geometryParameters["booleanTolerance"]);
  geometryParameters["copperEnvelopeShortFrame_y"] = (geometryParameters["copperEnvelopeJunction_y"] -
                                                      geometryParameters["copperEnvelopeJunction_innerRadius"] -
  ...
  geometryParameters["copperEnvelopeTongueTube_dPhi"] = (315. * deg -
  ...
  geometryParameters["copperEnvelopeTongueTube_z"] = (geometryParameters["copperEnvelopeHalfTube_z"] -
  ...
  geometryParameters["copperEnvelopeTongueStraightPart_z"] = (geometryParameters["copperEnvelopeHalfTube_z"] -
                                                              geometryParameters["copperEnvelopeHalfTube_halfLength"] +
  ...
  geometryParameters["copperEnvelopeTongueCurvedPart_phi"] = (geometryParameters["copperEnvelopeTongueCurvedPart_startPhi"] +
                                                              geometryParameters["copperEnvelopeTongueCurvedPart_dPhi"] / 2);
  geometryParameters["copperEnvelopeTongueCurvedPart_z"] = (geometryParameters["copperEnvelopeHalfTube_z"] -
                                                            geometryParameters["copperEnvelopeHalfTube_halfLength"] +
  ...
  // LXe
  geometryParameters["LXeMainPart_innerRadius"] = 0. * mm;
  geometryParameters["LXeMainPart_outerRadius"] = 125. * mm;
  geometryParameters["LXeMainPart_halfLength"] = 135.7 * mm;

  geometryParameters["LXeMainPart_x"] = 0. * mm;
  geometryParameters["LXeMainPart_y"] = 0. * mm;
  geometryParameters["LXeMainPart_z"] = 0. * mm;

  geometryParameters["LXePipeSubPart_innerRadius"] = 2.46 * mm;
  geometryParameters["LXePipeSubPart_outerRadius"] = 3.17 * mm;
  geometryParameters["LXePipeSubPart_halfLength"] = 95. * mm;

  geometryParameters["LXeMainPipeSubPart_rho"] = 95. * mm;
  geometryParameters["LXeMainPipeSubPart_phi"] = 37.5 * deg;
  geometryParameters["LXeMainPipeSubPart_z"] = ( // Same position as innCryoPipe
      geometryParameters["innCryoEnvelope_halfLength"] -
      geometryParameters["innCryoPipe_halfLength"] +
      2 * geometryParameters["innCryoTopSealUpperPart_halfWidth"]);

  geometryParameters["LXeColumnSubPart_innerRadius"] = 76.5 * mm;
  geometryParameters["LXeColumnSubPart_outerRadius"] = 78.5 * mm;
  geometryParameters["LXeColumnSubPart_halfLength"] = 224.7 * mm;

  geometryParameters["LXeMainColumnSubPart_x"] = 0. * mm;
  geometryParameters["LXeMainColumnSubPart_y"] = 0. * mm;
  geometryParameters["LXeMainColumnSubPart_z"] = ( // Same position as outCryoColumn
      geometryParameters["outCryoColumnSeal_z"] +
      geometryParameters["outCryoColumnSealLowerPart_halfWidth"] -
      geometryParameters["outCryoColumn_halfLength"] -
      geometryParameters["innCryoTranslation_z"]);

  geometryParameters["LXe_x"] = 0. * mm;
  geometryParameters["LXe_y"] = 0. * mm;
  geometryParameters["LXe_z"] = 0. * mm;
...
void XelabDetectorConstruction::ConstructCryostat()
{
  ...
  // LXe colour
  G4Colour LXeColour(0.7, 0.82, 0.98, 0.9); // 0.35
  G4VisAttributes *LXeVisAtt = new G4VisAttributes(LXeColour);
  LXeVisAtt->SetVisibility(true);

  // Inner cryostat colour
  // G4Colour innCryoColour(0.1, 0.5, 0.7, 0.8);
  G4Colour innCryoColour(1.0, 1.0, 1.0, 0.3);
  ...
  // LXe
  LXeMainPart_solid = new G4Tubs(
      "LXeMainPart_solid",
      geometryParameters["LXeMainPart_innerRadius"],
      geometryParameters["LXeMainPart_outerRadius"],
      geometryParameters["LXeMainPart_halfLength"],
      0. * deg, 360. * deg);

  LXePipeSubPart_solid = new G4Tubs(
      "LXePipeSubPart_solid",
      geometryParameters["LXePipeSubPart_innerRadius"],
      geometryParameters["LXePipeSubPart_outerRadius"],
      geometryParameters["LXePipeSubPart_halfLength"],
      0. * deg, 360. * deg);

  G4ThreeVector LXeMainPipeSubPart_pos;
  LXeMainPipeSubPart_pos.setRhoPhiZ(
      geometryParameters["LXeMainPipeSubPart_rho"],
      geometryParameters["LXeMainPipeSubPart_phi"],
      geometryParameters["LXeMainPipeSubPart_z"]);

  LXe_sub1_solid = new G4SubtractionSolid(
      "LXe_sub1_solid",
      LXeMainPart_solid,
      LXePipeSubPart_solid,
      0, LXeMainPipeSubPart_pos);

  LXeColumnSubPart_solid = new G4Tubs(
      "LXeColumnSubPart_solid",
      geometryParameters["LXeColumnSubPart_innerRadius"],
      geometryParameters["LXeColumnSubPart_outerRadius"],
      geometryParameters["LXeColumnSubPart_halfLength"],
      0. * deg, 360. * deg);

  G4ThreeVector LXeMainColumnSubPart_pos(
      geometryParameters["LXeMainColumnSubPart_x"],
      geometryParameters["LXeMainColumnSubPart_y"],
      geometryParameters["LXeMainColumnSubPart_z"]);

  LXe_solid = new G4SubtractionSolid(
      "LXe_solid",
      LXe_sub1_solid,
      LXeColumnSubPart_solid,
      0, LXeMainColumnSubPart_pos);

  LXe_logical = new G4LogicalVolume(
      LXe_solid, LXe,
      "LXe_logical");
  LXe_logical->SetVisAttributes(LXeVisAtt);

  G4ThreeVector LXe_pos(
      geometryParameters["LXe_x"],
      geometryParameters["LXe_y"],
      geometryParameters["LXe_z"]);

  LXe_physical = new G4PVPlacement(
      0, LXe_pos,
      LXe_logical,
      "LXe_physical",
